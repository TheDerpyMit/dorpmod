...
public class CigarItem extends Item {
   ...
   public void onUseTick(Level level, LivingEntity entity, ItemStack stack, int count) {
      if (level.isClientSide() && entity instanceof Player player && count % 2 == 0) {
         ...
         level.addParticle(ParticleTypes.LARGE_SMOKE, px, py, pz, player.getLookAngle().x * 0.05, 0.08, player.getLookAngle().z * 0.05);
      }
   }
   ...
   private void spawnSmokePuff(Level level, LivingEntity entity, int inhaleTicks) {
      if (level.isClientSide() && entity instanceof Player player) {
         int count = Math.min(40, inhaleTicks + 10);
         ...
         for (int i = 0; i < count; i++) {
            double vx = lookX * 0.15 + (random.nextDouble() - 0.5) * 0.1;
            double vy = lookY * 0.15 + (random.nextDouble() - 0.5) * 0.1 + 0.05;
            double vz = lookZ * 0.15 + (random.nextDouble() - 0.5) * 0.1;
            level.addParticle(ParticleTypes.LARGE_SMOKE, px, py, pz, vx, vy, vz);
         }
      }
   }
   ...
   public ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity entity) {
      if (level.isClientSide()) {
         this.spawnSmokePuff(level, entity, 64);
      }
   ...
   public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
      ...
      tooltipComponents.add(Component.literal("A premium quality cigar.").withStyle(new ChatFormatting[]{ChatFormatting.DARK_GRAY, ChatFormatting.ITALIC}));
      int usesLeft = stack.getMaxDamage() - stack.getDamageValue();
      tooltipComponents.add(Component.literal("Uses left: " + usesLeft).withStyle(ChatFormatting.GOLD));
   }
}
